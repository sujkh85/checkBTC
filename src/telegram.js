...
// 정확도 메시지 전송 함수
export async function sendAccuracyMessage({
  accuracyStats,
  currentPrediction,
  actualTrend,
  priceChange
...
export async function sendTrendAnalysisMessage({
  ...
  for (const [timeFrame, data] of Object.entries(timeFrameTrends)) {
    ...
    // 패턴 분석 정보 추가
    message += `*🔍 패턴 분석*\n`;
    const harmonicPatterns = Object.values(harmonic).filter(
      (v) => v && v.indexOf('없음') === -1
    );
  ...
  Object.entries(weightedTrends).forEach(([trend, score]) => {
    ...
    const scoreText =
      score === maxWeightedScore ? `${score}점⭐️` : `${score}점`;
