...
function analyzeOverallTrend(
  ...
  if (macdDiff < Math.abs(macd.signal * 0.1)) {
    // 10% 이내 차이면 횡보
    trendScores.횡보 += 2 * TREND_WEIGHTS.technical;
  ...
  else if (stoch.k >= 40 && stoch.k <= 60)
    trendScores.횡보 += 2 * TREND_WEIGHTS.technical;
  else trendScores.횡보 += 1 * TREND_WEIGHTS.technical;

  // OBV 분석 (횡보 조건 추가)
  if (Math.abs(obv) < 1000) {
    // OBV 변화가 작을 때 횡보로 판단
    trendScores.횡보 += 1 * TREND_WEIGHTS.technical;
  } else if (obv > 0) {
    trendScores.상승 += 1 * TREND_WEIGHTS.technical;
  ...
  if (maDiff < maCross.slow * 0.02) {
    // 2% 이내 차이로 완화
    trendScores.횡보 += 2 * TREND_WEIGHTS.technical;
  } else if (maDiff < maCross.slow * 0.03) {
    // 3% 이내도 약한 횡보로 판단
    trendScores.횡보 += 1 * TREND_WEIGHTS.technical;
  ...
  const hasBuySignal = harmonicPatterns.some((pattern) =>
    pattern.includes('매수')
  );
  const hasSellSignal = harmonicPatterns.some((pattern) =>
    pattern.includes('매도')
  );
  ...
  // 3. 일목구름표 분석 (가중치 6)
  const hasBullishSignal = ichimoku.signals.some(
    (signal) =>
      signal.includes('상향돌파') ||
      signal.includes('구름대 위') ||
      signal.includes('선행스팬 상향')
  );
  const hasBearishSignal = ichimoku.signals.some(
    (signal) =>
  ...
  try {
    ...
    if (currentWave && typeof currentWave === 'object') {
      ...
      if (phase && typeof phase === 'string') {
        if (
          phase === '상승 파동 진행 중' ||
          phase === '상승 5파 완성 후 조정 국면'
        ) {
          trendScores.상승 += 1 * TREND_WEIGHTS.elliott;
        } else if (
          phase === '하락 파동 진행 중' ||
          phase === '하락 5파 완성 후 조정 국면'
        ) {
          trendScores.하락 += 1 * TREND_WEIGHTS.elliott;
        }
      }
  ...
  if (fibonacci.currentLevel) {
    ...
    if (fibonacci.nextLevel) {
      const distanceToNext =
        (fibonacci.nextLevel.price - prices[prices.length - 1].close) /
  ...
  const finalTrend = Object.entries(trendScores).find(
  ...
  if (totalScore > 0) {
    ...
    const maxTimeframeWeight = Math.max(
      ...Object.values(TREND_WEIGHTS.timeframe)
    );

    // 최종 강도 계산 (0-100 범위로 제한)
    trendStrength = Math.min(
      100,
      Math.max(0, (baseStrength * timeframeWeight) / maxTimeframeWeight)
    ).toFixed(1);
  }
  ...
  // 현재 가격이 레벨 사이에 있는 경우
    ...
    for (let i = 0; i < levelEntries.length - 1; i++) {
      ...
      if (
        ...
        prices[prices.length - 1].close < nextPrice
      ) {
        currentLevel = {
