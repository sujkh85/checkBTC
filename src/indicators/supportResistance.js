...
function analyzeBreakout(currentPrice, supportLevels, resistanceLevels) {
    ...
    if (supportLevels.length > 0) {
        ...
        result.distanceToSupport = ((currentPrice - result.nearestSupport) / result.nearestSupport * 100).toFixed(2);
    ...
    if (resistanceLevels.length > 0) {
        ...
        result.distanceToResistance = ((result.nearestResistance - currentPrice) / currentPrice * 100).toFixed(2);
...
function analyzeSupportResistance(prices) {
    ...
    const analysis = analyzeBreakout(currentPrice, supportLevels, resistanceLevels);
