...
function analyzeIchimokuSignals(currentPrice, conversionLine, baseLine, leadingSpanA, leadingSpanB, laggingSpan) {
    ...
    // 강력한 매매 신호 확인
    const strongSignals = [];
    if (currentPrice > conversionLine && currentPrice > baseLine && 
        currentPrice > leadingSpanA && currentPrice > leadingSpanB) {
        strongSignals.push('강력한 상승 신호');
    } else if (currentPrice < conversionLine && currentPrice < baseLine && 
               currentPrice < leadingSpanA && currentPrice < leadingSpanB) {
